...
//退回2次
public class BackTwoFragment extends BaseFragment {
...
//    private List<LocalMedia> jlImgList = new ArrayList<>();
    ...
    public int getLayoutResId() {
        return R.layout.fragment_back_two;
    }
        ...
        HttpRequest.get(ctx).url(ServerInterface.listBackToTheRecord).params("id", id).doPost(new HttpStringCallBack() {
            ...
            public void onSuccess(Object result) {
                ...
                if (bean != null && bean.code == 0) {
                    if (listEmpty(bean.list) && bean.list.size() > 1) {

                        try {
                            tv_back_sgdw.setText(bean.list.get(1).constructionUnit);
                        } catch (Exception e) {

                        }

                        try {
                            tv_back_shiGongRen.setText(bean.list.get(1).constructionDirector);
                        } catch (Exception e) {

                        }
                        try {
                            tv_shiGongBackTime.setText(bean.list.get(1).creationTime);
                        } catch (Exception e) {

                        }

                        try {
                            tv_shiGongBackNote.setText(bean.list.get(1).transferExplain);
                        } catch (Exception e) {
...
//                        }
                        try {
                            setPicRcv(back_note_list, bean.list.get(1).childTransferRecord);
                        } catch (Exception e) {

                        }
                        tv_danWei.setText(bean.list.get(1).backType);
                        tv_back_danWeiPeople.setText(bean.list.get(1).backUsername);
                        tv_jlBack_Date.setText(bean.list.get(1).backCreationTime);
                        tv_jlBack_Status.setText("已退回");
                        tv_jlBack_Status.setTextColor(setColor(R.color.red2));
                        tv_JLBackReason.setText(bean.list.get(1).backCause);
...
//                        }
                        try {
                            setPicRcv(back_reason_list, bean.list.get(1).childbackPictureVideo);
                        } catch (Exception e) {
