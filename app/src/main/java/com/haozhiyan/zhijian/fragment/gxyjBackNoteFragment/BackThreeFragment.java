...
//退回3次
public class BackThreeFragment extends BaseFragment {
...
//    private List<LocalMedia> jlImgList = new ArrayList<>();
    ...
    public int getLayoutResId() {
        return R.layout.fragment_back_three;
    }
        ...
        HttpRequest.get(ctx).url(ServerInterface.listBackToTheRecord).params("id", id).doPost(new HttpStringCallBack() {
            ...
            public void onSuccess(Object result) {
                ...
                if (bean != null && bean.code == 0) {
                    if (listEmpty(bean.list) && bean.list.size() > 2) {

                        try {
                            tv_back_sgdw.setText(bean.list.get(2).constructionUnit);
                        } catch (Exception e) {

                        }

                        try {
                            tv_back_shiGongRen.setText(bean.list.get(2).constructionDirector);
                        } catch (Exception e) {

                        }
                        try {
                            tv_shiGongBackTime.setText(bean.list.get(2).creationTime);
                        } catch (Exception e) {

                        }

                        try {
                            tv_shiGongBackNote.setText(bean.list.get(2).transferExplain);
                        } catch (Exception e) {

                        }

                        tv_shiGongBackStatus.setText("申请验收");
//                        if (listEmpty(bean.list.get(1).childTransferRecord)) {
//                            for (String path : bean.list.get(1).childTransferRecord) {
...
//                        }
                        try {
                            setPicRcv(back_note_list, bean.list.get(2).childTransferRecord);
                        } catch (Exception e) {

                        }
                        tv_danWei.setText(bean.list.get(2).backType);
                        tv_back_danWeiPeople.setText(bean.list.get(2).backUsername);
                        tv_jlBack_Date.setText(bean.list.get(2).backCreationTime);
                        tv_jlBack_Status.setText("已退回");
                        tv_jlBack_Status.setTextColor(setColor(R.color.red2));
                        tv_JLBackReason.setText(bean.list.get(2).backCause);
//                        if (listEmpty(bean.list.get(1).childbackPictureVideo)) {
//                            for (String path : bean.list.get(1).childbackPictureVideo) {
//                                LocalMedia localMedia = new LocalMedia();
//                                localMedia.setPath(ServerInterface.PVUrl() + path);
//                                localMedia.setPictureType(PVAUtils.getFileLastType(path));
...
//                        }
                        try {
                            setPicRcv(back_reason_list, bean.list.get(2).childbackPictureVideo);
                        } catch (Exception e) {
